...
from utils import get_embedding, find_crop_in_global
...
def run_refinement_loop(
    ...
    begin_pos_vis, starting_images, orig_embeds, att_mask, position_ids, vis_len
):
    ...
    orig_embeds, prob_after, _, boxes_previous, boxes, ents_start, g_max, _ = refine_big(
        ...
        final=False, samples=4, mode=mode, tokens = args.token_number,
    )
    ...
    while True:
        ...
        orig_embeds2, prob_before, prob_after2, boxes_previous, boxes, ents, g_max, _ = refine_big(
            model, input_ids, image_tensor, image_sizes,
            orig_embeds.clone(), att_mask, position_ids,
            dataset, index, begin_pos_vis,
            ...
            final=False, samples=1, mode=mode, pad=False,tokens = args.token_number
        )
...
def eval_model(args):
    ...
    if args.load_data:
        textvqa_val = "/cluster/scratch/mgroepl/data/textvqa/val.json"
        box_data, id_to_index, ordered_ids = load_gt_bbox_data(textvqa_val, textvqa_val)
        index_data = -1
    ...
    with open(answers_file, "w") as ans_file:
        for idx, ((input_ids, image_tensor, image_sizes), line) in enumerate(
            ...
            if args.load_data and str(ordered_ids[idx]) in id_to_index:
                index_data += 1
                bb = box_data[index_data]["bbox"]
                box_dataset_orig_size = (bb[2] * bb[3]) / (image_sizes[0][0] * image_sizes[0][1])
            ...
            orig_embeds, boxes_previous, start_ent, g_max = run_refinement_loop(
                ...
                orig_embeds, att_mask, position_ids, vis_len,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, default="attention", choices=["attention", "grad"])
