...
def _save_grad_plot(grad_map, iteration, label=""):
    plt.imshow(grad_map.cpu().numpy(), cmap="viridis")
    plt.colorbar()
    plt.savefig(f"/cluster/scratch/mgroepl/debug/test/{iteration}tensor_image{label}.png")
    plt.close()


def _save_concatenated_images(images, iteration):
    ...
    for img in images:
        ...
        x += img.width
    canvas.save(f"/cluster/scratch/mgroepl/debug/test/{iteration}concatenated_image.png")


def _plot_grad_results(grad_flat, grad_grid, is_multi, vis_len, add, boxes_previous, iteration):
    if is_multi:
        _save_grad_plot(grad_grid.reshape(48, 48), iteration)
        _save_grad_plot(grad_flat[:vis_len].reshape(24, 24), iteration, label="Single")
    else:
        _save_grad_plot(grad_flat.reshape(24 * add, 24), iteration)
    _save_concatenated_images(boxes_previous, iteration)
...
def refine_big(
    ...
    plot=False, add=1, mode="grad", final=False, samples=1, pad=False,tokens = 1
):
    """
    ...
    if mode == "grad":
        ...
        if plot:
            _plot_grad_results(grad_flat, grad_grid, is_multi, vis_len, add, boxes_previous, iteration)
    else:
    ...
    if plot:
        orig_img.save("/cluster/scratch/mgroepl/debug/test/orig1.png")
    ...
    if plot:
        ...
        crop_images[best_idx].save(f"/cluster/scratch/mgroepl/debug/test/{iteration}final.png")
